...
RUN_NAME="${RUN_NAME:-sdxl_cloud_teacher_watch75_v7_focus}"
PROMPTS="${PROMPTS:-configs/sdxl_tiny_teacher_prompts_v7_focus_watch_freeprompt.json}"
LIMIT="${LIMIT:-0}"
...
MAX_BORDER_EDGE_DENSITY="${MAX_BORDER_EDGE_DENSITY:-0.30}"
MAX_FOREGROUND_COMPONENTS="${MAX_FOREGROUND_COMPONENTS:-6}"
MIN_LARGEST_FOREGROUND_COMPONENT_RATIO="${MIN_LARGEST_FOREGROUND_COMPONENT_RATIO:-0.50}"
OUT_DIR="${OUT_DIR:-datasets/${RUN_NAME}}"

python3 tools/build_watch_v7_focus_prompt_config.py --out "$PROMPTS"
...
echo "Focused V7 dataset ready: $OUT_DIR"
