...
OUTPUT_DIR="$ROOT_DIR/artifacts/ios-models/sd15/512"
...
if ! has_hf_token; then
  ...
  echo "Set HF_TOKEN (or login with huggingface-cli) after accepting the SD 1.5 model license." >&2
  exit 1
fi

exec "$PYTHON_BIN" "$ROOT_DIR/scripts/convert_stable_diffusion.py" \
